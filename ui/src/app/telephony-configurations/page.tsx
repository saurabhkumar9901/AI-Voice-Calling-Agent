...
export default function ConfigureTelephonyPage() {
  ...
  const [callLogsUrl, setCallLogsUrl] = useState("");
  ...
  useEffect(() => {
    ...
    // Fetch existing configuration with retry logic to handle auth token
    // settling on new devices (e.g., after AWS deployment, first login on new PC).
    const fetchConfig = async (retriesLeft = 2) => {
      ...
      try {
        ...
        if (retriesLeft > 0) {
          // Retry after a short delay — auth token may not have settled yet
          await new Promise(resolve => setTimeout(resolve, 1000));
          return fetchConfig(retriesLeft - 1);
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchCallLogsUrl = async () => {
      try {
        ...
        console.error("Failed to fetch provider call logs URL:", error);
      }
    };
    fetchCallLogsUrl();
  }, [getAccessToken, authLoading, user]);

  const handleSaveCallLogsUrl = async () => {
    const trimmed = callLogsUrl.trim();
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error((data as { detail?: string })?.detail || "Failed to save URL");
      }
      setCallLogsUrl(trimmed);
      toast.success("Provider call logs URL saved");
    } catch (error) {
    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-8">
              ...
              <CardHeader>
                ...
                <CardDescription>
                  {selectedProvider === "telnyx" ? (
                    ...
                    <>
                       Connect ENXT AI to your Asterisk PBX using the Asterisk REST Interface (ARI).
                       ARI provides a WebSocket-based event model for controlling calls via Stasis applications.
                    </>
                  ) : selectedProvider === "cloudonix" ? (
                    <>
                       Cloudonix is an AI Connectivity platform, enabling you to connect ENXT AI to any SIP product or SIP Telephony Provider.<br/><br/>
                      <iframe
                        style={{ border: 0 }}
                        width="100%"
                        height="450"
                  ...
                  <div className="space-y-4 text-sm">
                    <div>
                      ...
                      <ol className="list-decimal list-inside space-y-1 text-muted-foreground">
                        ...
                        <li>Ensure the ARI HTTP endpoint is accessible from ENXT AI</li>
                  ...
                  <div className="space-y-4 text-sm">
                    <div>
                      ...
                      <ol className="list-decimal list-inside space-y-1 text-muted-foreground">
                        ...
                        <li>Configure your Cloudoinx <i>API Token</i> and <i>Cloudonix Domain Name</i> in ENXT AI</li>
                        <li>Configure an optional outbound phone number for your ENXT AI agent</li>
                      </ol>
                    </div>
                    <div className="bg-muted border border-border rounded p-3">
                      <p className="text-sm">
        ...
        <div className="mt-6">
          <Card>
            <CardHeader>
              <CardTitle>Provider Call Logs</CardTitle>
              <CardDescription>
                Paste your telephony provider&apos;s call logs page URL (e.g. the Vobiz voice logs page).
                A shortcut button to this page appears beside Download CSV on the Reports page.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex flex-col sm:flex-row gap-2">
                <Input
                  id="call_logs_url"
                  placeholder="https://console.vobiz.ai/app/voice/logs"
                  value={callLogsUrl}
                  onChange={(e) => setCallLogsUrl(e.target.value)}
                />
                <Button
                  type="button"
                  onClick={handleSaveCallLogsUrl}
