...
import type { CreateToolRequest } from "@/client/types.gen";
        ...
        try {
            ...
            const response = await createToolApiV1ToolsPost({
                body: {
                    ...
                    definition: createToolDefinition(newToolCategory) as CreateToolRequest["definition"],
                },
                headers: {
                    Authorization: `Bearer ${accessToken}`,
                },
            ...
            if (response.error) {
                const rawDetail = (response.error as { detail?: unknown })?.detail;
                // FastAPI 422 details arrive as an array of {type, loc, msg, ...}
                // objects — flatten to text so React never renders an object.
                const errorDetail = Array.isArray(rawDetail)
                    ? rawDetail
                          .map((d) =>
                              typeof d === "string"
                                  ? d
                                  : (d as { msg?: string })?.msg || "Validation error",
                          )
                          .join("; ")
