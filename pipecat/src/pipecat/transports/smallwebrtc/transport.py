    ...
    async def read_audio_frame(self):
        ...
        while True:
            ...
            try:
                ...
                _timeout_count += 1
                if _timeout_count <= 5 or _timeout_count % 50 == 0:
                    logger.warning(f"[AUDIO-DIAG] Timeout #{_timeout_count}: No audio frame received within 2s")
                if (
                    self._webrtc_connection.is_connected()
            ...
            _frame_count += 1
            if _frame_count <= 5 or _frame_count % 500 == 0:
                # Log first 5 frames and every 500th frame after
                arr = frame.to_ndarray()
                max_val = int(abs(arr).max()) if arr.size > 0 else 0
                logger.info(
                    f"[AUDIO-DIAG] Frame #{_frame_count}: rate={frame.sample_rate}Hz, "
                    f"samples={frame.samples}, format={frame.format.name}, max_amplitude={max_val}"
                )
