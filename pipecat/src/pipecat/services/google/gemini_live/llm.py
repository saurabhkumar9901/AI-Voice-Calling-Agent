...
class GeminiLiveLLMService(LLMService):
    ...
    async def _handle_msg_model_turn(self, msg: LiveServerMessage):
        ...
        if not msg.server_content.model_turn:
            return
        ...
        for part in msg.server_content.model_turn.parts:
            ...
            if text:
                ...
                if part.thought:
                    ...
                    # Push TTSTextFrame to support RealtimeFeedbackObserver live chat UI streaming
            ...
            # If we have no audio, continue to the next part.
        ...
        # Check for grounding metadata in server content outside the parts loop
    ...
    async def _handle_msg_turn_complete(self, message: LiveServerMessage):
        ...
        if self._bot_is_responding:
            await self._set_bot_is_responding(False)
            if self._settings.modalities.value == "AUDIO":
                # AUDIO modality case (even if there's text streaming, we still need to stop TTS)
                await self.push_frame(TTSStoppedFrame())
