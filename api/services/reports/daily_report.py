...
from api.utils.call_report import (
    coerce_summary,
    extract_provider_call_info,
    extract_user_says,
    public_artifact_url,
)
from api.utils.transcript import generate_transcript_text

RECORDING_LINK_EXPIRATION_SECONDS = 7 * 24 * 3600


async def _public_link(run_id: int, artifact_type: str, has_artifact: bool) -> str:
    """Absolute public download URL for a run artifact (works from any browser)."""
    if not has_artifact:
        return ""
    try:
        token = await db_client.ensure_public_access_token(run_id)
        if token:
            return public_artifact_url(artifact_type, token)
    except Exception as e:
        logger.warning(f"[run {run_id}] Failed to build public link: {e}")
    return ""


async def _full_transcript_text(
    transcript_url: Optional[str], transcript_events: list
) -> str:
    """Full two-sided transcript text.

    Prefers the stored transcript file (regenerated from the recording, both
    speakers); falls back to text built from live log events.
    """
    if transcript_url:
        with tempfile.NamedTemporaryFile(delete=False) as tmp:
            local_path = tmp.name
        try:
            if await storage_fs.adownload_file(transcript_url, local_path):
                with open(local_path, encoding="utf-8") as f:
                    text = f.read().strip()
                if text:
                    return text
        except Exception as e:
            logger.warning(f"Failed to fetch stored transcript {transcript_url}: {e}")
        finally:
            if os.path.exists(local_path):
                try:
                    ...
                    pass
    try:
        return generate_transcript_text(transcript_events or []).strip()
    except Exception:
        return ""
...
class DailyReportService:
    ...
    async def get_daily_runs_detail(
        ...
        # Format runs for CSV export (unified: our AI data + provider detail)
        detailed_runs = []
        for run in runs:
            ...
            provider = extract_provider_call_info(run.get("provider_callbacks"))

            call_type = run.get("call_type")
            call_type = getattr(call_type, "value", call_type) or ""

            transcript_text = await _full_transcript_text(
                run.get("transcript_url"), run.get("transcript_events")
            )
            ...
            detailed_runs.append(
                {
                    ...
                    "summary": coerce_summary(run.get("call_summary")),
                    "customer_says": extract_user_says(transcript_text),
                    "action_items": "; ".join(str(a) for a in action_items),
                    "sentiment": coerce_summary(run.get("sentiment")),
                    "transcript": transcript_text,
                    "recording_link": await _public_link(
                        run["id"], "recording", bool(run.get("recording_url"))
                    ),
                    "transcript_link": await _public_link(
                        run["id"], "transcript", bool(run.get("transcript_url"))
                    ),
                    **provider,
                }
            )
