...
class RealtimeFeedbackObserver(BaseObserver):
    ...
    def __init__(
        ...
        persist_assistant_text: bool = False,
    ):
        """
        Args:
            ...
            persist_assistant_text: When True, accumulate TTSTextFrame text and
                       flush it to the logs buffer as BOT_TEXT at turn boundaries
                       (BotStoppedSpeaking / interruption / next user turn / call
                       end). Needed for S2S pipelines where the assistant context
                       aggregator never completes a turn, so on_assistant_turn_stopped
                       never fires and assistant speech is lost from transcripts.
        """
        super().__init__()
        self._ws_sender = ws_sender
        self._logs_buffer = logs_buffer
        self._persist_assistant_text = persist_assistant_text
        self._assistant_text_parts: list[str] = []
        self._assistant_frames_seen = False
    ...
    async def _accumulate_assistant_text(self, text: str):
        """Accumulate assistant TTS text for later persistence (S2S pipelines)."""
        if self._persist_assistant_text and text:
            if not self._assistant_frames_seen:
                self._assistant_frames_seen = True
                logger.info("S2S assistant text frames flowing through pipeline")
            self._assistant_text_parts.append(text)

    async def _flush_assistant_text(self, reason: str):
        """Flush accumulated assistant text to the logs buffer as one BOT_TEXT event.

        Idempotent: flushing with an empty accumulator is a no-op, so multiple
        turn-boundary triggers cannot double-log.
        """
        if not self._persist_assistant_text:
            return
        text = "".join(self._assistant_text_parts).strip()
        self._assistant_text_parts = []
        if not text:
            return
        logger.info(f"Persisting assistant turn ({len(text)} chars, via {reason})")
    ...
    async def on_push_frame(self, data: FramePushed):
        ...
        if isinstance(frame, (EndFrame, CancelFrame, StopFrame)):
            await self._cancel_clock_task()
            await self._flush_assistant_text("pipeline-end")
        # Handle interruptions - clear any queued bot text
        elif isinstance(frame, InterruptionFrame):
            await self._handle_interruption()
            await self._flush_assistant_text("interruption")
        # Bot speaking state - WS only (ephemeral state signals, not persisted)
            ...
            await self._flush_assistant_text("bot-stopped-speaking")
            await self._send_ws(
        ...
        # Complete turn text is persisted via register_turn_handlers
            ...
            await self._flush_assistant_text("user-turn")
            await self._send_ws(
                {
                    "type": RealtimeFeedbackType.USER_TRANSCRIPTION.value,
                    "payload": {
        ...
        # Complete turn text is persisted via register_turn_handlers, plus via
        # the assistant-text accumulator for S2S pipelines (see __init__).
        elif isinstance(frame, TTSTextFrame):
            await self._accumulate_assistant_text(frame.text)
            message = {
                "type": RealtimeFeedbackType.BOT_TEXT.value,
                "payload": {
                    "text": frame.text,
