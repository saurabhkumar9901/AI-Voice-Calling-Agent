...
from pipecat.frames.frames import Frame, LLMContextFrame, LLMMessagesAppendFrame, TTSSpeakFrame
...
def register_event_handlers(
    ...
    async def maybe_trigger_initial_response():
        ...
        If the start node has a greeting configured, play it directly via TTS for traditional
        pipelines, or inject it as an assistant message for S2S pipelines.
        ...
        if (
            ...
            if greeting:
                ...
                is_s2s = isinstance(engine.llm, GeminiLiveLLMService)
                
                if is_s2s:
                    logger.debug(
                        "Both pipeline_started and client_connected received - injecting greeting prompt for S2S"
                    )
                    # For S2S, prompt the model to say the greeting exactly
                    prompt = f"Please start the conversation by saying exactly this: {greeting}"
                    engine.context.add_message({"role": "user", "content": prompt})
