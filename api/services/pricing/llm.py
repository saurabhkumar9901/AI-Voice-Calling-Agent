...
LLM_PRICING: Dict[str, Dict[str, TokenPricingModel]] = {
    ...
    ServiceProviders.GOOGLE: {
        "gemini-3.1-flash-live-preview": TokenPricingModel(
            prompt_token_price=Decimal("3.00") / 1000000,  # $3.00 per 1M tokens (audio input) - or $0.005/min alt; $0.75/1M text, $1.00/1M image/video
            completion_token_price=Decimal("12.00") / 1000000,  # $12.00 per 1M tokens (audio output incl. thinking) - or $0.018/min alt; $4.50/1M text
        ),
        "gemini-3.5-flash-lite": TokenPricingModel(
            prompt_token_price=Decimal("0.75") / 1000000,  # $0.75 per 1M tokens (text) - update if official Gemini 3.5 pricing differs; audio fallback $3.00/1M
            completion_token_price=Decimal("4.50") / 1000000,  # $4.50 per 1M tokens (text incl. thinking) - audio fallback $12.00/1M
        ),
        "default": TokenPricingModel(
            prompt_token_price=Decimal("3.00") / 1000000,  # Fallback for Google S2S/live models
            completion_token_price=Decimal("12.00") / 1000000,
        ),
    },
