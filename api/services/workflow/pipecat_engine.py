...
class PipecatEngine:
    def __init__(
        ...
        *,
        ...
        embeddings_provider: Optional[str] = "openai",
        ...
        self._embeddings_provider: str = embeddings_provider or "openai"
    ...
    async def _update_llm_context(self, system_prompt: str, functions: list[dict]):
        ...
        if isinstance(self.llm, GeminiLiveLLMService):
            # For Gemini Live S2S, the system instruction is part of the
            # LiveConnectConfig sent when the WebSocket session is established.
            # _update_settings() stores values in memory but does NOT apply
            # them to the active connection.  We must:
            #   1. Set _system_instruction_from_init (read by _connect())
            #   2. Update tools so _connect() picks them up
            #   3. Reconnect to establish a new session with the new config
            self.llm._system_instruction_from_init = system_prompt

            if functions:
                tools_schema = ToolsSchema(standard_tools=functions)
                self.context.set_tools(tools_schema)
                adapter = self.llm.get_llm_adapter()
                self.llm._tools_from_init = adapter.from_standard_tools(tools_schema)

            if getattr(self.llm, "_session", None) is not None:
                await self.llm._reconnect()
        else:
    ...
    async def _register_knowledge_base_function(
        ...
        async def retrieve_kb_func(function_call_params: FunctionCallParams) -> None:
            ...
            try:
                ...
                if not self._embeddings_api_key and self._embeddings_provider != "self_hosted":
                    raise ValueError(
    ...
    async def _setup_llm_context(self, node: Node) -> None:
        ...
        # Detect S2S pipeline by checking if LLM is GeminiLiveLLMService
        from pipecat.services.google.gemini_live import GeminiLiveLLMService
        is_s2s = isinstance(self.llm, GeminiLiveLLMService)
