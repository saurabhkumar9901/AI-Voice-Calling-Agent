...
def tool_to_function_schema(tool: Any) -> Dict[str, Any]:
    ...
    # If this is a schedule_callback tool, ensure the callback time parameter.
    if definition.get("type") == "schedule_callback":
        if "callback_at_iso" not in properties:
            properties["callback_at_iso"] = {
                "type": "string",
                "description": (
                    "When to call the user back, as an ISO-8601 datetime "
                    "(e.g. '2026-09-20T16:00:00+05:30'). Resolve relative "
                    "requests like 'after 4 PM' or 'next Tuesday' against the "
                    "current date and time before calling."
                ),
            }
            required.append("callback_at_iso")
