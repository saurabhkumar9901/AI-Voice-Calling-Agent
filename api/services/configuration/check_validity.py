...
class UserConfigurationValidator:
    def __init__(self):
        self._validator_map = {
            ...
            ServiceProviders.AWS_POLLY.value: self._check_aws_polly_api_key,
            ServiceProviders.KOKORO.value: self._check_kokoro_api_key,
    ...
    async def validate(
        ...
        is_s2s = getattr(configuration, "active_pipeline", "traditional") == "s2s"

        status_list.extend(
            self._validate_service(
                configuration.llm, "llm", required=not is_s2s, skip_missing=skip_missing
            )
        )
        status_list.extend(
            self._validate_service(
                configuration.stt, "stt", required=not is_s2s, skip_missing=skip_missing
            )
        )
        status_list.extend(
            self._validate_service(
                configuration.tts, "tts", required=not is_s2s, skip_missing=skip_missing
            )
        )
        status_list.extend(
            self._validate_service(
                configuration.s2s, "s2s", required=is_s2s, skip_missing=skip_missing
            )
        )
    ...
    def _check_aws_bedrock_api_key(self, model: str, service_config) -> bool:
        # AWS Bedrock hasn't migrated to api_key yet in this branch
        if not getattr(service_config, "aws_access_key", None) or not getattr(service_config, "aws_secret_key", None):
            raise ValueError("AWS access key and secret key are required for Bedrock")
        return True

    def _check_aws_polly_api_key(self, model: str, api_key: str) -> bool:
        return True

    def _check_kokoro_api_key(self, model: str, api_key: str) -> bool:
        return True
