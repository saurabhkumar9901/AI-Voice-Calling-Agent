...
class VobizProvider(TelephonyProvider):
    ...
    async def verify_webhook_signature(
        ...
        """
        Verify Vobiz webhook signature for security using V2 protocol.

        Vobiz (Plivo-compatible) uses HMAC-SHA256 signature verification V2:
        - Header: x-vobiz-signature-v2 (HMAC-SHA256 hash)
        - Header: x-vobiz-signature-v2-nonce (random nonce)
        - Expected Signature = Base64(HMAC-SHA256(auth_token, url + nonce))
        """
        import base64
        import hashlib
        import hmac

        if not signature_v2 or not nonce:
            logger.warning("Missing signature-v2 or nonce headers for Vobiz webhook")
            return False
        ...
        try:
            # Create expected signature: Base64(HMAC-SHA256(auth_token, url + nonce))
            payload = f"{url}{nonce}"
            ...
            is_valid = hmac.compare_digest(expected_signature, signature_v2)

            if not is_valid:
                logger.warning(
                    f"Vobiz webhook signature mismatch. URL: {url}, Nonce: {nonce}, Expected: {expected_signature}, Got: {signature_v2}"
                )
...
<Response>
    ...
    def validate_account_id(config_data: dict, webhook_account_id: str) -> bool:
        ...
        # Vobiz inbound webhooks may not contain ParentAuthID/AuthID parameters.
        # If not provided, we permit validation to succeed here and rely on the
        # destination phone number matching the organization configuration.
    ...
    async def verify_inbound_signature(
        ...
        return await self.verify_webhook_signature(
            url, webhook_data, signature, nonce, body
        )
