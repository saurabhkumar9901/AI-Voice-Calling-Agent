...
async def get_backend_endpoints() -> tuple[str, str]:
    ...
    if BACKEND_API_ENDPOINT:
        ...
        try:
            from api.constants import BACKEND_WS_ENDPOINT
            ...
            if scheme:
                ...
                ws_url = BACKEND_WS_ENDPOINT.rstrip("/") if BACKEND_WS_ENDPOINT else BACKEND_API_ENDPOINT.rstrip("/").replace(scheme, ws_scheme, 1)
            else:
                http_url = "http://" + BACKEND_API_ENDPOINT.rstrip("/")
                ws_url = BACKEND_WS_ENDPOINT.rstrip("/") if BACKEND_WS_ENDPOINT else "ws://" + BACKEND_API_ENDPOINT.rstrip("/")
