...
class TunnelURLProvider:
    ...
    _check_interval: int = 60  # Only retry every 60 seconds if failed
    ...
    async def get_tunnel_urls(cls) -> tuple[str, str]:
        ...
        """
        # Return cached URLs immediately if already discovered
        if cls._cached_urls is not None:
            return cls._cached_urls

        try:
            # Throttle checks if we recently failed
            current_time = time.time()
            if current_time - cls._last_check_time < cls._check_interval:
                raise ValueError("Throttling cloudflared check after recent failure")
    ...
    async def _get_cloudflared_urls(cls) -> Optional[tuple[str, str]]:
        ...
        try:
            ...
            logger.debug("Timeout connecting to cloudflared metrics endpoint")
            return None
        except aiohttp.ClientError as e:
            logger.debug(f"Error connecting to cloudflared: {e}")
            return None
        except Exception as e:
