...
async def _validate_inbound_request(
    ...
    x_vobiz_signature_v2: str = None,
    x_vobiz_signature_v2_nonce: str = None,
    ...
    if x_twilio_signature or x_vobiz_signature_v2 or x_cx_apikey or telnyx_signature:
        backend_endpoint, _ = await get_backend_endpoints()
        ...
        elif provider_class.PROVIDER_NAME == "vobiz" and x_vobiz_signature_v2:
            logger.info(f"Verifying Vobiz signature for URL: {webhook_url}")
            signature_valid = await provider_instance.verify_inbound_signature(
                webhook_url,
                webhook_data,
                x_vobiz_signature_v2,
                x_vobiz_signature_v2_nonce,
                webhook_body,
            )
        ...
        if ENVIRONMENT == "local" and not signature_valid:
            logger.warning("Local environment detected: signature verification failed but bypassed for testing.")
            signature_valid = True
...
async def _validate_organization_provider_config(
    ...
    if not account_id and provider_class.PROVIDER_NAME != "vobiz":
        logger.warning(
...
async def _process_status_update(workflow_run_id: int, status: StatusCallbackRequest):
    ...
    # Follow-up callback outcome tracking: a connected call marks the callback
    # completed; busy/failed outcomes consume the single retry (handled in
    # callback_tasks). No-answer/voicemail are terminal by design.
    try:
        callback_id = (workflow_run.gathered_context or {}).get("callback_id")
        if callback_id:
            from api.tasks.callback_tasks import handle_callback_attempt_outcome

            if status.status == "completed":
                await handle_callback_attempt_outcome(int(callback_id), "completed")
                ...
                await handle_callback_attempt_outcome(
                    int(callback_id), status.status, status.status
                )
    except Exception as e:
        logger.warning(
            f"[run {workflow_run_id}] Callback outcome hook failed: {e}"
        )
...
async def handle_vobiz_hangup_callback(
    workflow_run_id: int,
    request: Request,
    x_vobiz_signature_v2: Optional[str] = Header(None),
    x_vobiz_signature_v2_nonce: Optional[str] = Header(None),
):
    ...
    if x_vobiz_signature_v2:
        ...
        from api.constants import ENVIRONMENT
        if not is_valid and ENVIRONMENT == "local":
            logger.warning(f"[run {workflow_run_id}] Local environment: bypassing invalid Vobiz hangup signature")
            is_valid = True
...
async def handle_vobiz_hangup_callback_by_workflow(
    ...
    if x_vobiz_signature_v2:
        ...
        from api.constants import ENVIRONMENT
        if not is_valid and ENVIRONMENT == "local":
            logger.warning(f"[workflow {workflow_id}] Local environment: bypassing invalid Vobiz hangup signature")
            is_valid = True
...
async def handle_inbound_telephony(
    ...
    try:
        ...
        logger.info(f"Vobiz signature V2 header: {x_vobiz_signature_v2}")
        logger.info(f"Vobiz nonce header: {x_vobiz_signature_v2_nonce}")

        webhook_body = ""
        if provider_class.PROVIDER_NAME == "vobiz":
            raw_body = await request.body()
            webhook_body = raw_body.decode("utf-8")
            logger.info(f"Vobiz inbound call - Body: {webhook_body}")
