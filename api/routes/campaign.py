...
class ColumnMappingRequest(BaseModel):
    """Excel header mapping for Name / Phone Number / City columns."""

    name_col: str = Field(..., min_length=1)
    phone_col: str = Field(..., min_length=1)
    city_col: str = Field(..., min_length=1)


class CreateCampaignRequest(BaseModel):
    name: str = Field(..., min_length=1, max_length=255)
    workflow_id: int
    source_type: str = Field(..., pattern="^(google-sheet|csv|excel)$")
    source_id: str  # Google Sheet URL or CSV/Excel file key
    ...
    column_mapping: Optional[ColumnMappingRequest] = None
...
async def create_campaign(
    ...
    # Excel sources resolve columns through the user-provided mapping.
    mapped_headers: Optional[List[str]] = None
    mapped_rows: Optional[List[List[str]]] = None
    if request.source_type == "excel":
        if not request.column_mapping:
            raise HTTPException(
                status_code=400,
                detail="column_mapping (name_col, phone_col, city_col) is required for Excel sources",
            )
        from api.services.campaign.sources.excel import ExcelSyncService

        excel_service = (
            sync_service
            if isinstance(sync_service, ExcelSyncService)
            else ExcelSyncService()
        )
        try:
            mapped_headers, mapped_rows = await excel_service.get_mapped_data(
                request.source_id, request.column_mapping.model_dump()
            )
        except ValueError as e:
            raise HTTPException(status_code=400, detail=str(e))

    # Validate template variables against source data columns.
    # For Excel sources the mapped headers/rows are used instead of raw ones.
    template_headers = mapped_headers or validation_result.headers
    template_rows = mapped_rows or validation_result.rows
...
async def preview_excel_source(
    ...
    """Return Excel headers + sample rows so the UI can offer column mapping.

    The file key must belong to the user's organization and point to an
    .xlsx/.xls workbook previously uploaded through the direct upload endpoint.
    """
    if not request.file_key.startswith(f"campaigns/{user.selected_organization_id}/"):
        raise HTTPException(
            status_code=403,
            detail="Access denied: file does not belong to your organization",
        )
    if not request.file_key.lower().endswith((".xlsx", ".xls")):
        raise HTTPException(
            status_code=400, detail="Only .xlsx/.xls workbooks are supported"
        )

    from api.services.campaign.sources.excel import ExcelSyncService

    try:
        preview = await ExcelSyncService().get_preview(request.file_key)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def delete_campaign_endpoint(
    ...
    """Delete a campaign and its queued runs (call history is preserved).
    ...
    try:
        await db_client.delete_campaign(campaign_id, user.selected_organization_id)
    except ValueError as e:
...
async def get_campaign_source_download_url(
    ...
    """Get presigned download URL for campaign CSV/Excel source file

    Only works for file-based source types. For Google Sheets, use the source_id directly.
    ...
    # Only generate download URL for file-based sources
    if campaign.source_type not in ("csv", "excel"):
        raise HTTPException(
            status_code=400,
            detail=f"Download URL only available for CSV/Excel sources. This campaign uses {campaign.source_type}",
        )
