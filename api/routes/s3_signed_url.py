...
@router.post(
    "/upload-csv-direct",
    response_model=PresignedUploadUrlResponse,
    summary="Upload CSV/Excel directly through API",
)
async def upload_csv_direct(
    file: UploadFile = File(...),
    user=Depends(get_user),
):
    """Upload a CSV or Excel file directly through the API (server-side proxy to storage).
    ...
    Access Control:
    * All authenticated users can upload CSV/Excel files scoped to their organization.
    """
    ...
    try:
        ...
        if not filename.lower().endswith(ALLOWED_EXTENSIONS):
            raise HTTPException(
                status_code=400,
                detail="Only CSV (.csv) and Excel (.xlsx, .xls) files are accepted",
            )
        ...
        if file_size > MAX_FILE_SIZE:
            raise HTTPException(
                status_code=400,
                detail=f"File size ({file_size} bytes) exceeds maximum ({MAX_FILE_SIZE} bytes). For larger lists, split the file or contact support."
            )
        ...
        logger.info(
            f"Uploading CSV '{filename}' ({file_size} bytes) to storage, "
        ...
        logger.info(
            f"File uploaded directly for user {user.id}, org {user.selected_organization_id}, file_key: {file_key}"
        )
        ...
        logger.error(f"Error in direct file upload: {exc}")
        raise HTTPException(
