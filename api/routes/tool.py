...
class HttpApiConfig(BaseModel):
    ...
    customMessage: Optional[str] = Field(
        default=None, description="Custom message to play before executing the tool"
    )
...
class ScheduleCallbackToolDefinition(BaseModel):
    """Tool definition for Schedule Callback tools (no configuration —
    the agent provides callback_at_iso and an optional note at runtime)."""

    schema_version: int = Field(default=1, description="Schema version")
    type: Literal["schedule_callback"] = Field(description="Tool type")
    config: Dict[str, Any] = Field(
        default_factory=dict, description="Schedule Callback configuration"
    )
...
ToolDefinition = Annotated[
    Union[
        ...
        ScheduleCallbackToolDefinition,
    ],
