...
@router.post(
    ...
    summary="Upload document directly through API",
)
async def upload_document_direct(
    file: UploadFile = File(...),
    user=Depends(get_user),
):
    """Upload a document directly through the API (server-side proxy to storage).

    This endpoint handles the entire upload flow in one request:
    1. Accepts the file via multipart form data
    2. Uploads it to S3/MinIO server-side (no browser-to-storage connection needed)
    3. Creates a document record in the database
    4. Enqueues background processing (chunking + embedding)

    Use this endpoint instead of the presigned URL flow when the storage backend
    is not directly accessible from the browser (e.g., MinIO behind HTTP in a
    cloud deployment served over HTTPS).
    ...
    * All authenticated users can upload documents scoped to their organization.
    ...
    try:
        ...
        if file_ext not in ACCEPTED_EXTENSIONS:
            raise HTTPException(
                status_code=400,
                detail=f"Unsupported file type '{file_ext}'. Accepted: {', '.join(sorted(ACCEPTED_EXTENSIONS))}"
            )
        ...
        # Generate unique document UUID and S3 key
        document_uuid = str(uuid.uuid4())
        s3_key = f"knowledge_base/{user.selected_organization_id}/{document_uuid}/{filename}"
        mime_type = file.content_type or "application/octet-stream"

        # Upload to storage server-side
        logger.info(
            f"Uploading document '{filename}' ({file_size} bytes) to storage, "
            f"user {user.id}, org {user.selected_organization_id}"
        )

        file_stream = io.BytesIO(content)
        # acreate_file expects a file-like object with async read()
        # Wrap BytesIO to add async read support
        class AsyncBytesIO:
        ...
        logger.info(
            f"Created document {document_uuid} (id={document.id}) via direct upload "
            f"and enqueued processing for org {user.selected_organization_id}"
        )
        ...
        logger.error(f"Error in direct upload: {exc}")
        raise HTTPException(
    ...
    try:
        ...
        if embeddings_provider == ServiceProviders.GOOGLE:
            ...
            embedding_service = OpenAIEmbeddingService(
                db_client=db_client,
                api_key=(embeddings_api_key or "local-key-placeholder") if is_self_hosted else embeddings_api_key,
