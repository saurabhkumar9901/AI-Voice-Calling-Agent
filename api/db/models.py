...
class WorkflowRunModel(Base):
    ...
    # Post-call summary generated from the transcript using the agent's LLM.
    call_summary = Column(Text, nullable=True)
    call_summary_generated_at = Column(DateTime(timezone=True), nullable=True)
    # Post-call insights: action items (JSON list) and caller sentiment
    # ("Interested" | "Not interested" | "Neutral", dedicated local NLP model).
...
class CallbackModel(Base):
    """Scheduled follow-up call requested by a caller during a conversation.

    Created live by the ``schedule_callback`` agent tool or post-call by
    transcript extraction. A background scheduler places the call at
    ``scheduled_for``; on busy/failed outcomes a single retry is scheduled
    (+5 minutes, or next 08:00 in the caller's timezone during quiet hours).
    """
    ...
    parent_callback_id = Column(
        Integer, ForeignKey("callbacks.id", ondelete="SET NULL"), nullable=True
    )
    ...
    organization = relationship("OrganizationModel")
    workflow = relationship("WorkflowModel")
    parent_callback = relationship("CallbackModel", remote_side=[id])

    __table_args__ = (
        Index("ix_callbacks_due", "state", "scheduled_for"),
        Index("ix_callbacks_organization_id", "organization_id"),
    )
